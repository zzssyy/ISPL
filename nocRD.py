...
def parse_args():
    ...
    parser.add_argument("-m", type=int, help="nocRD features top n", default=-1)
...
class Dim_Rd(object):
    ...
    def run(self, inputfile):
        ...
        features_sorted, features = nocrd_run(file, self.logger)
        self.read_data()
        if int(args.e) == -1:
...
def nocrd_run(filecsv, logger):
    logger.info('nocrd start...')
    ...
    nocrd = sorted(mrmrValue, key=lambda x: x[1], reverse=True)  # 按mrmrValue 由大到小排序
    feature = {}
    i = 1
    for x in features_name:
        if x == 'class' or x == '0':
            ...
            i += 1
    print(feature)
    logger.info('nocrd end.')
    return nocrd, feature
